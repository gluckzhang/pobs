...
ELASTIC_CERT_PATH = "/home/gluckzhang/development/elastdocker/secrets/certs/ca/ca.crt"
ELASTIC_USERNAME = "elastic"
ELASTIC_PASSWORD = "changeme"
ELASTIC_LOG_INDEX = ".ds-logs-generic-default-2022.05.11-000001"
ELASTIC_APM_INDEX = "apm-7.15.2-metric-000001"
...
def perf_instructions(container_name, seconds):
    ...
    perf_cmd = "perf stat -e instructions --timeout %d -p %s 2>&1"%(seconds*1000, pids) # --timeout takes miliseconds as input
    cpu_instructions = -1
    try:
        ...
        match = pattern_cpu_instructions.search(top_output)
...
def run_original_image(image_name):
    with tempfile.NamedTemporaryFile(mode="w+b") as stdout_f, tempfile.NamedTemporaryFile(mode="w+b") as stderr_f:
        ...
        cpu_instructions = -1
        if continuously_running: cpu_instructions = perf_instructions(container_name, 60)
...
def query_elasticsearch(container_name):
    ...
    try:
        resp = ELASTIC_CLIENT.search(index=ELASTIC_LOG_INDEX, size=1000, query={
            "bool": {
                "must": [
                    {"match": {"container.name":container_name}},
    ...
    try:
        resp = ELASTIC_CLIENT.search(index=ELASTIC_APM_INDEX, query={
            "bool": {
                "must": [
                    {"match": {"service.name":container_name}},
                    {"range": {"@timestamp": {"gte": "now-5m", "lt": "now"}}}
                ]
            }
        })
...
def main():
    ...
    with open(OPTIONS.dataset, 'rt') as file:
        ...
        for project in projects:
            if "is_able_to_run" in project: continue
