...
def draw_violin_plots(data, labels):
    ...
    figure, axs = plt.subplots(figsize=(9, 3), nrows=1, ncols=len(data))
    ...
    for i in range(len(data)):
        ...
        for pc in violinplot["bodies"]:
            ...
            pc.set_alpha(1)
        axs[i].text(1.02, quartile1, percentage_str(quartile1), va='center', fontsize=12)
        axs[i].text(1.02, median, percentage_str(median), va='center', fontsize=12)
        axs[i].text(1.02, quartile3, percentage_str(quartile3), va='center', fontsize=12)
        axs[i].yaxis.set_major_formatter(lambda x, pos: 0 if x == 0 else "%.0f%%"%(x*100))
        axs[i].set_ylim([max(whiskers_min, 0), whiskers_max])
...
def main():
    ...
    with open(options.filepath, 'rt') as file:
        ...
        logging.info("cpu_usage_increase (min, median, max): %.2f, %.2f, %.2f"%(min(cpu_usage_increase), numpy.median(cpu_usage_increase), max(cpu_usage_increase)))
        ...
        overhead_data = [image_size_increase, memory_usage_increase, cpu_instructions_increase]
        overhead_labels = ["Image Size", "Memory", "CPU Instructions"]
